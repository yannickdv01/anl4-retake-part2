...
function parseArguments {
	...
	for arg in "${args[@]}"; do
		...
		if [ "${arg:0:1}" == "-" ] || [ "$wasPrevValid" -eq 1 ]; then
			...
			echo "ERROR: $arg is not a valid argument. The script will now exit for safety reasons." >&2
			bash
		fi
	done
        ...
        case "${opt}" in
            d)
                ...
                if [ -z "$dirReport" ]; then
                    ...
                    print "Error: cant set dirreport twice. The script will now exit for safety reasons."
                    log "Error: dirreport is already set to $dirReport."
                    bash
                fi
                #check if dirreport is a directory
                if [ ! -d "$dirReport" ]; then
                    print "Error: $dirReport is not a directory. The script will now exit for safety reasons."
                    log "Error: $dirReport is not a directory."
                    bash
                fi
                ;;
            b)
                ...
                if [ -z "$badWords" ]; then
                    ...
                    print "Error: cant set badwords twice. The script will now exit for safety reasons."
                    log "Error: badwords is already set to $badWords"
                    bash
                fi
                #check if badwords is a file
                if [ ! -f "$badWords" ]; then
                    print "Error: $badWords is not a file. The script will now exit for safety reasons."
                    log "Error: $badWords is not a file."
                    bash
                fi
                ;;
            :)
                ...
                print "ERROR: At least 1 valid argument has to be supplied. The script will now exit for safety reasons." >&2
                bash
                ;;
            \?)
                #cancelExecution=1
                #print "Requires option"
                print "ERROR: -$OPTARG is not a valid option. The script will now exit for safety reasons." >&2
                bash
                ;;
        esac
    done
...
function readBadWords {
    if [ -n "$1" ]; then
        ...
        if [ -f "$1" ]; then
            ...
            print "ERROR: Badwords file does not exist. The script will now exit for safety reasons." >&2
...
function configureBB {  
    ...
    if [ "$dirReport" == "" ]; then
        ...
        print "ERROR: '$dirReport' is not an existing directory. The script will now exit for safety reasons." >&2
